...
export default function SimpleImage({ src, alt, className = "", aspectRatio = "auto" }: SimpleImageProps) {
  ...
  if (!src || imageError) {
    return (
      <div className={`bg-gray-100 flex items-center justify-center ${getAspectRatioClass()} ${className}`}>
    ...
    <div className={`relative overflow-hidden ${getAspectRatioClass()} ${className}`}>
      {isLoading && (
        <div className="absolute inset-0 bg-gray-100 flex items-center justify-center">
          <div className="animate-pulse text-gray-400 text-xs">Yuklanmoqda...</div>
        </div>
      )}
      <img
